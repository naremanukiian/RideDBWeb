...
const ROLES = {
  passenger: {
    ...
    nav:[
      ...
      { group:'Interactive' },
      { id:'p-demo',   label:'Live Demo',   ico:ICO.bolt },
    ],
    home:'p-rides',
  },
  driver: {
    ...
    nav:[
      ...
      { group:'Interactive' },
      { id:'d-demo',    label:'Live Demo',  ico:ICO.bolt },
    ],
    home:'d-trips',
  },
  analyst: {
    ...
    nav:[
      ...
      { group:'Interactive' },
      { id:'a-demo',     label:'Live Demo',   ico:ICO.bolt },
    ],
    home:'a-overview',
  },
  dba: {
    ...
    nav:[
      ...
      { group:'Interactive' },
      { id:'dba-demo',   label:'Live Demo',      ico:ICO.bolt },
    ],
    home:'dba-dash',
  },
};
...
const TITLES = {
  ...
  'p-demo':     ['Live Demo','Interactive SQL — ride_app permissions'],
  'd-demo':     ['Live Demo','Interactive SQL — ride_app permissions'],
  'a-demo':     ['Live Demo','Interactive SQL — ride_report (SELECT only)'],
  'dba-demo':   ['Live Demo','Interactive SQL — ride_dba (full control)'],
...
function selectRole(role) {
  ...
  // init demo if it exists in this role
  setTimeout(() => { demoRefreshStats(); demoRefreshDropdowns(); demoSwitchTab('rides'); }, 50);
}
...
function showView(id, btn) {
  ...
  // refresh demo if navigating to it
  if (id.endsWith('-demo')) {
    setTimeout(() => { demoRefreshStats(); demoRefreshDropdowns(); demoSwitchTab('rides'); }, 30);
  }
}
...
function buildPassenger(pg) {
  ...
  pg.appendChild(v4);
  // Live Demo
  const vDemo = V('p-demo');
  ht(vDemo, buildLiveDemo('passenger'));
  pg.appendChild(vDemo);
}
...
function buildDriver(pg) {
  ...
  const vDemoD = V('d-demo');
  ht(vDemoD, buildLiveDemo('driver'));
  pg.appendChild(vDemoD);
}
...
function buildAnalyst(pg) {
  ...
  const vDemoA = V('a-demo');
  ht(vDemoA, buildLiveDemo('analyst'));
  pg.appendChild(vDemoA);
}
...
function buildDBA(pg) {
  ...
  // Live Demo
  const vDemoDBA = V('dba-demo');
  ht(vDemoDBA, buildLiveDemo('dba'));
  pg.appendChild(vDemoDBA);
...
// ════════════════════════════════════════════════════════════════
// LIVE DEMO — in-memory SQL engine, role-aware
// ════════════════════════════════════════════════════════════════

// Shared mutable state — persists across role switches in same session
const DEMO = {
  ...
  nextRide:   DB.rides.length + 1,
  nextUser:   DB.users.length + 1,
  nextPay:    DB.payments.length + 1,
  nextRating: DB.ratings.length + 1,
  log: [],
};

function demoLog(type, msg) {
  const t = new Date().toLocaleTimeString('en-US',{hour12:false,hour:'2-digit',minute:'2-digit',second:'2-digit'});
  DEMO.log.push({ type, msg, t });
  const con = document.getElementById('demo-console');
  if (!con) return;
  const el = document.createElement('div');
  el.className = 'demo-log';
  el.innerHTML = `<span class="dl-time">${t}</span><span class="dl-${type}">${msg}</span>`;
  con.appendChild(el);
...
function demoLogSep() {
  ...
  if (!con) return;
  const el = document.createElement('div');
  el.className = 'demo-sep';
  con.appendChild(el);
}

// ── trigger simulations ───────────────────────────────────────
function demo_trg_br1(uid) {
  demoLog('trg','🔥 trg_no_concurrent_rides — INSTEAD OF INSERT ON rides');
  const has = DEMO.rides.find(r => r.uid===uid && r.status==='Pending');
  if (has) {
    demoLog('err',`  RAISERROR: User #${uid} already has Pending ride #${has.id}. RETURN.`);
    return false;
  }
  demoLog('ok','  BR-1 ✓ — no active Pending ride for this user');
  return true;
}
function demo_trg_br6(did) {
  demoLog('trg','🔥 trg_driver_busy_on_ride — AFTER INSERT ON rides');
  const d = DEMO.drivers.find(x=>x.id===did);
  if (d) { d.status='Busy'; demoLog('ok',`  SET drivers.Status = 'Busy' → ${d.first} ${d.last}`); }
}
function demo_trg_br5(rid, start, end) {
  demoLog('trg','🔥 trg_calc_duration — AFTER UPDATE ON rides');
  const dur = Math.round((new Date(end)-new Date(start))/60000);
  const r = DEMO.rides.find(x=>x.id===rid);
  if (r) r.dur = dur;
  demoLog('ok',`  SET RideDuration = DATEDIFF(MINUTE, StartTime, EndTime) = ${dur} min`);
  return dur;
}
function demo_trg_br7(did) {
  demoLog('trg','🔥 trg_driver_available_on_complete — AFTER UPDATE ON rides');
  const d = DEMO.drivers.find(x=>x.id===did);
  if (d) { d.status='Available'; demoLog('ok',`  SET drivers.Status = 'Available' → ${d.first} ${d.last}`); }
}
function demo_trg_br4(did) {
  demoLog('trg','🔥 trg_update_driver_rating — AFTER INSERT ON ratings');
  const rideIds = new Set(DEMO.rides.filter(r=>r.did===did).map(r=>r.id));
  const rs = DEMO.ratings.filter(r=>rideIds.has(r.rid));
  if (!rs.length) return;
  const avg = Math.round(rs.reduce((s,r)=>s+r.dr,0)/rs.length*100)/100;
  const d = DEMO.drivers.find(x=>x.id===did);
  if (d) { d.rating=avg; demoLog('ok',`  SET drivers.Rating = ROUND(AVG(DriverRating),2) = ${avg} → ${d.first} ${d.last}`); }
}
function demo_trg_br3(rid) {
  demoLog('trg','🔥 trg_validate_payment_ride — INSTEAD OF INSERT ON payments');
  const r = DEMO.rides.find(x=>x.id===rid);
  if (r && r.status==='Cancelled') {
    demoLog('err',`  RAISERROR: Ride #${rid} is Cancelled — payment BLOCKED. RETURN.`); return false;
  }
  demoLog('ok',`  BR-3 ✓ — ride #${rid} is '${r?.status}', payment allowed`); return true;
}
function demo_trg_br2(rid) {
  demoLog('trg','🔥 trg_prevent_delete_completed — AFTER DELETE ON rides');
  const r = DEMO.rides.find(x=>x.id===rid);
  if (r && r.status==='Completed') {
    demoLog('err','  RAISERROR: Completed rides cannot be deleted.');
    demoLog('err','  ROLLBACK TRANSACTION — delete undone.'); return false;
  }
  return true;
}

// ── helpers ───────────────────────────────────────────────────
const DU = id => { const u=DEMO.users.find(x=>x.id===id); return u?u.first+' '+u.last:'—'; };
const DD = id => { const d=DEMO.drivers.find(x=>x.id===id); return d?d.first+' '+d.last:'—'; };
const DL = id => { const l=DEMO.locations.find(x=>x.id===id); return l?l.name:'—'; };
function demoBadge(v) {
  if (!v) return '<span style="color:var(--t3)">NULL</span>';
  return `<span class="badge badge-${v}">${v}</span>`;
}

// ── table renderer ────────────────────────────────────────────
function demoRenderTable(name) {
  const rows = DEMO[name] || [];
  const cols = {
    rides:    ['ID','Passenger','Driver','From','To','Fare','Status','Dur','Promo'],
    drivers:  ['ID','Name','Licence','Rating','Status'],
    users:    ['ID','Name','Email','Phone','Registered'],
    payments: ['ID','Ride','Amount','Method','Date','Status'],
    ratings:  ['ID','Ride','Driver★','User★'],
    promos:   ['ID','Code','Discount','Expires'],
  };
  if (!cols[name]) return '';
  let h = `<div class="demo-tbl-wrap"><table class="tbl"><thead><tr>`;
  cols[name].forEach(c => h += `<th>${c}</th>`);
  h += '</tr></thead><tbody>';
  rows.forEach(r => {
    h += '<tr>';
    if (name==='rides') h += `
      <td>#${String(r.id).padStart(3,'0')}</td>
      <td style="white-space:nowrap;font-size:11px">${DU(r.uid)}</td>
      <td style="white-space:nowrap;font-size:11px">${DD(r.did)}</td>
      <td style="font-size:10px">${DL(r.slid)}</td>
      <td style="font-size:10px">${DL(r.elid)}</td>
      <td style="color:var(--green)">$${Number(r.fare).toFixed(2)}</td>
      <td>${demoBadge(r.status)}</td>
      <td style="color:var(--t3)">${r.dur!=null?r.dur+'m':'—'}</td>
      <td style="font-size:10px;color:var(--gold)">${r.promo?'P-'+r.promo:'—'}</td>`;
    else if (name==='drivers') h += `
      <td>${r.id}</td><td>${r.first} ${r.last}</td>
      <td style="font-size:10px;color:var(--t3)">${r.lic}</td>
      <td style="color:var(--gold)">${r.rating}★</td>
      <td>${demoBadge(r.status)}</td>`;
    else if (name==='users') h += `
      <td>${r.id}</td><td>${r.first} ${r.last}</td>
      <td style="font-size:10px">${r.email}</td>
      <td style="color:var(--t3)">${r.phone||'NULL'}</td>
      <td style="font-size:10px;color:var(--t3)">${r.reg}</td>`;
    else if (name==='payments') h += `
      <td>${r.id}</td><td>#${r.rid}</td>
      <td style="color:var(--green)">$${Number(r.amount).toFixed(2)}</td>
      <td>${demoBadge(r.method)}</td>
      <td style="font-size:10px;color:var(--t3)">${r.date}</td>
      <td>${demoBadge(r.status)}</td>`;
    else if (name==='ratings') h += `
      <td>${r.id}</td><td>#${r.rid}</td>
      <td>${'★'.repeat(r.dr)}</td>
      <td>${r.ur?'★'.repeat(r.ur):'<span style="color:var(--t3)">NULL</span>'}</td>`;
    else if (name==='promos') h += `
      <td>${r.id}</td>
      <td style="color:var(--gold);font-weight:700">${r.code}</td>
      <td style="color:var(--green)">−${r.discount}%</td>
      <td style="font-size:10px;color:var(--t3)">${r.expiry}</td>`;
    h += '</tr>';
  });
  h += '</tbody></table></div>';
  return h;
...
function demoRefreshTable(name) {
  const el = document.getElementById('demo-tbl-body');
  if (el) el.innerHTML = demoRenderTable(name);
  demoRefreshStats();
  demoRefreshDropdowns();
}

function demoRefreshStats() {
  const el = document.getElementById('demo-stats');
  if (!el) return;
  const rev = DEMO.rides.filter(r=>r.status==='Completed').reduce((s,r)=>s+r.fare,0);
  const av  = DEMO.drivers.filter(d=>d.status==='Available').length;
  el.innerHTML = `
    <div class="demo-stat"><span>${DEMO.users.length}</span>Users</div>
    <div class="demo-stat"><span>${av}</span>Available</div>
    <div class="demo-stat"><span>${DEMO.rides.filter(r=>r.status==='Pending').length}</span>Pending</div>
    <div class="demo-stat"><span>${DEMO.rides.filter(r=>r.status==='Completed').length}</span>Completed</div>
    <div class="demo-stat"><span style="color:var(--green)">$${rev.toFixed(2)}</span>Revenue</div>
    <div class="demo-stat"><span>${DEMO.payments.length}</span>Payments</div>
    <div class="demo-stat"><span>${DEMO.ratings.length}</span>Ratings</div>
  `;
}

function demoRefreshDropdowns() {
  const fill = (id, items, vFn, tFn, empty) => {
    const el = document.getElementById(id); if (!el) return;
    el.innerHTML = empty ? `<option value="">${empty}</option>` : '';
    items.forEach(i => { const o=document.createElement('option'); o.value=vFn(i); o.textContent=tFn(i); el.appendChild(o); });
  };
  fill('dm-book-uid', DEMO.users,   i=>i.id, i=>`${i.first} ${i.last} (#${i.id})`);
  fill('dm-book-did', DEMO.drivers.filter(d=>d.status==='Available'), i=>i.id, i=>`${i.first} ${i.last} (${i.rating}★)`);
  fill('dm-book-slid',DEMO.locations,i=>i.id,i=>`${i.name} · ${i.city}`);
  fill('dm-book-elid',DEMO.locations,i=>i.id,i=>`${i.name} · ${i.city}`);
  const pending = DEMO.rides.filter(r=>r.status==='Pending');
  fill('dm-comp-rid',  pending, i=>i.id, i=>`#${i.id} — ${DU(i.uid)}`, '(none)');
  fill('dm-cancel-rid',pending, i=>i.id, i=>`#${i.id} — ${DU(i.uid)}`, '(none)');
  const completed = DEMO.rides.filter(r=>r.status==='Completed');
  const ratedIds  = new Set(DEMO.ratings.map(r=>r.rid));
  fill('dm-rate-rid',  completed.filter(r=>!ratedIds.has(r.id)), i=>i.id, i=>`#${i.id} — ${DU(i.uid)} $${i.fare}`, '(none)');
  fill('dm-promo-rid', completed, i=>i.id, i=>`#${i.id} — $${Number(i.fare).toFixed(2)}`, '(none)');
  fill('dm-pay-rid',   DEMO.rides.filter(r=>!DEMO.payments.find(p=>p.rid===r.id)), i=>i.id, i=>`#${i.id} — ${i.status} — ${DU(i.uid)}`, '(none)');
  fill('dm-del-rid',   DEMO.rides, i=>i.id, i=>`#${i.id} ${i.status} — ${DU(i.uid)}`);
  fill('dm-promo-code',DEMO.promos.filter(p=>new Date(p.expiry)>new Date()), i=>i.id, i=>`${i.code} (−${i.discount}%)`);
  fill('dm-ur-uid',    DEMO.users, i=>i.id, i=>`${i.first} ${i.last} (#${i.id})`);
  // update table tab counts
  ['rides','drivers','users','payments','ratings'].forEach(t => {
    const tab = document.getElementById('dm-tab-'+t);
    if (tab) tab.textContent = `${t} (${DEMO[t].length})`;
  });
}

// ── procedure handlers (called from inline onclick) ───────────
window.demoOps = {
  register() {
    const first=document.getElementById('dm-reg-first').value.trim();
    const last =document.getElementById('dm-reg-last').value.trim();
    const email=document.getElementById('dm-reg-email').value.trim();
    const phone=document.getElementById('dm-reg-phone').value.trim();
    if (!first||!last||!email){demoLog('err','All fields required');return;}
    if (DEMO.users.find(u=>u.email===email)){demoLog('err','RAISERROR: Email already exists — UNIQUE violation');return;}
    demoLogSep();
    demoLog('sql','EXEC sp_register_user');
    demoLog('sql',`  INSERT INTO users VALUES ('${first}','${last}','${email}','${phone||'NULL'}',GETDATE())`);
    const id=DEMO.nextUser++;
    DEMO.users.push({id,first,last,email,phone,reg:new Date().toISOString().slice(0,10)});
    demoLog('ok',`  → NewUserID = ${id} (SCOPE_IDENTITY())`);
    demoRefreshTable('users'); demoSwitchTab('users');
  },
  availDrivers() {
    const city=document.getElementById('dm-avail-city').value;
    demoLogSep();
    demoLog('sql','EXEC sp_available_drivers'+(city?` @City='${city}'`:''));
    const avail=DEMO.drivers.filter(d=>d.status==='Available');
    demoLog('ok',`  → ${avail.length} available driver(s)`);
    avail.forEach(d=>demoLog('ok',`    #${d.id} ${d.first} ${d.last} · Rating:${d.rating}`));
    demoSwitchTab('drivers');
  },
  userRides() {
    const uid=parseInt(document.getElementById('dm-ur-uid').value);
    demoLogSep();
    demoLog('sql',`EXEC sp_get_user_rides @UserID=${uid}`);
    const rs=DEMO.rides.filter(r=>r.uid===uid);
    demoLog('ok',`  → ${rs.length} ride(s) for ${DU(uid)}`);
    rs.forEach(r=>demoLog('ok',`    #${r.id} ${DL(r.slid)}→${DL(r.elid)} $${r.fare} ${r.status}`));
    demoSwitchTab('rides');
  },
  bookRide() {
    const uid =parseInt(document.getElementById('dm-book-uid').value);
    const did =parseInt(document.getElementById('dm-book-did').value);
    const slid=parseInt(document.getElementById('dm-book-slid').value);
    const elid=parseInt(document.getElementById('dm-book-elid').value);
    const fare=parseFloat(document.getElementById('dm-book-fare').value);
    if(!uid||!did||!slid||!elid||isNaN(fare)){demoLog('err','All fields required');return;}
    demoLogSep();
    demoLog('sql',`INSERT INTO rides (UserID,DriverID,StartLocationID,EndLocationID,Fare,Status)`);
    demoLog('sql',`VALUES (${uid},${did},${slid},${elid},${fare},'Pending')`);
    if(!demo_trg_br1(uid)){demoLogSep();return;}
    const id=DEMO.nextRide++;
    const now=new Date().toISOString().replace('T',' ').slice(0,16);
    const vid=DEMO.drivers.find(d=>d.id===did)?.vid||did;
    DEMO.rides.push({id,uid,did,vid,slid,elid,start:now,end:null,fare,status:'Pending',dur:null,promo:null});
    demoLog('ok',`  → RideID #${id} inserted — Status='Pending'`);
    demo_trg_br6(did);
    demoRefreshTable('rides'); demoSwitchTab('rides');
  },
  completeRide() {
    const rid =parseInt(document.getElementById('dm-comp-rid').value);
    const fare=parseFloat(document.getElementById('dm-comp-fare').value);
    const r=DEMO.rides.find(x=>x.id===rid);
    if(!r){demoLog('err','No pending ride selected');return;}
    demoLogSep();
    demoLog('sql',`EXEC sp_complete_ride @RideID=${rid}, @Fare=${fare}`);
    const now=new Date().toISOString().replace('T',' ').slice(0,16);
    r.status='Completed'; r.end=now; r.fare=fare;
    demoLog('ok',`  Ride #${rid} → Completed`);
    demo_trg_br5(rid, r.start, now);
    demo_trg_br7(r.did);
    demoRefreshTable('rides'); demoSwitchTab('rides');
  },
  cancelRide() {
    const rid=parseInt(document.getElementById('dm-cancel-rid').value);
    const r=DEMO.rides.find(x=>x.id===rid);
    if(!r){demoLog('err','No pending ride selected');return;}
    demoLogSep();
    demoLog('sql',`EXEC sp_cancel_ride @RideID=${rid}`);
    r.status='Cancelled';
    demoLog('ok',`  Ride #${rid} → Cancelled`);
    demo_trg_br7(r.did);
    demoRefreshTable('rides'); demoSwitchTab('rides');
  },
  applyPromo() {
    const rid=parseInt(document.getElementById('dm-promo-rid').value);
    const pid=parseInt(document.getElementById('dm-promo-code').value);
    const r=DEMO.rides.find(x=>x.id===rid);
    const p=DEMO.promos.find(x=>x.id===pid);
    if(!r||!p){demoLog('err','Select ride and promo');return;}
    demoLogSep();
    demoLog('sql',`EXEC sp_apply_promo @RideID=${rid}, @PromoID=${pid}, @NewFare OUTPUT`);
    if(new Date(p.expiry)<new Date()){demoLog('err','RAISERROR: Promo expired');return;}
    const oldFare=r.fare;
    const newFare=Math.round(oldFare*(1-p.discount/100)*100)/100;
    r.fare=newFare; r.promo=pid;
    demoLog('ok',`  @NewFare OUTPUT = $${newFare.toFixed(2)} (was $${oldFare.toFixed(2)}, −${p.discount}%)`);
    document.getElementById('dm-promo-out').textContent='→ $'+newFare.toFixed(2);
    demoRefreshTable('rides'); demoSwitchTab('rides');
  },
  insertPayment() {
    const rid=parseInt(document.getElementById('dm-pay-rid').value);
    const amt=parseFloat(document.getElementById('dm-pay-amount').value);
    const mth=document.getElementById('dm-pay-method').value;
    if(!rid||isNaN(amt)){demoLog('err','Select ride and amount');return;}
    demoLogSep();
    demoLog('sql',`INSERT INTO payments (RideID,Amount,Method,Status) VALUES (${rid},${amt},'${mth}','Paid')`);
    if(!demo_trg_br3(rid)){demoLogSep();return;}
    const id=DEMO.nextPay++;
    DEMO.payments.push({id,rid,amount:amt,method:mth,date:new Date().toISOString().slice(0,10),status:'Paid'});
    demoLog('ok',`  → PaymentID #${id} — $${amt.toFixed(2)} via ${mth}`);
    demoRefreshTable('payments'); demoSwitchTab('payments');
  },
  insertRating() {
    const rid=parseInt(document.getElementById('dm-rate-rid').value);
    const dr =parseInt(document.getElementById('dm-rate-dr').value);
    const urV=document.getElementById('dm-rate-ur').value;
    const ur =urV?parseInt(urV):null;
    const r=DEMO.rides.find(x=>x.id===rid);
    if(!r){demoLog('err','No unrated ride selected');return;}
    if(DEMO.ratings.find(x=>x.rid===rid)){demoLog('err','RAISERROR: UNIQUE(RideID) — already rated');return;}
    demoLogSep();
    demoLog('sql',`INSERT INTO ratings (RideID,DriverRating,UserRating) VALUES (${rid},${dr},${ur||'NULL'})`);
    const id=DEMO.nextRating++;
    DEMO.ratings.push({id,rid,dr,ur,comment:null});
    demoLog('ok',`  → RatingID #${id}`);
    demo_trg_br4(r.did);
    demoRefreshTable('ratings'); demoSwitchTab('ratings');
  },
  deleteRide() {
    const rid=parseInt(document.getElementById('dm-del-rid').value);
    const r=DEMO.rides.find(x=>x.id===rid);
    if(!r){demoLog('err','Select a ride');return;}
    demoLogSep();
    demoLog('sql',`DELETE FROM rides WHERE RideID=${rid}`);
    demoLog('warn',`  (Ride #${rid} status: '${r.status}')`);
    if(!demo_trg_br2(rid)){demoLogSep();return;}
    DEMO.rides.splice(DEMO.rides.findIndex(x=>x.id===rid),1);
    demoLog('ok',`  Ride #${rid} deleted (${r.status} — not Completed)`);
    demoRefreshTable('rides'); demoSwitchTab('rides');
  },
  monthlyRevenue() {
    const yr=parseInt(document.getElementById('dm-rev-year').value);
    const mo=parseInt(document.getElementById('dm-rev-month').value);
    demoLogSep();
    demoLog('sql',`EXEC sp_monthly_revenue @Year=${yr}, @Month=${mo}`);
    const rs=DEMO.rides.filter(r=>{
      if(r.status!=='Completed')return false;
      const d=new Date(r.start);
      return d.getFullYear()===yr&&d.getMonth()+1===mo;
    });
    if(!rs.length){demoLog('warn',`  → No completed rides for ${yr}-${String(mo).padStart(2,'0')}`);return;}
    const tot=rs.reduce((s,r)=>s+r.fare,0);
    demoLog('ok',`  TotalRides: ${rs.length} | TotalRevenue: $${tot.toFixed(2)} | AvgFare: $${(tot/rs.length).toFixed(2)}`);
    demoLog('ok',`  MinFare: $${Math.min(...rs.map(r=>r.fare)).toFixed(2)} | MaxFare: $${Math.max(...rs.map(r=>r.fare)).toFixed(2)}`);
  },
};

function demoSwitchTab(name) {
  document.querySelectorAll('.demo-tab').forEach(t=>t.classList.toggle('active', t.dataset.t===name));
  demoRefreshTable(name);
}

function demoClearConsole() {
  const el=document.getElementById('demo-console'); if(el) el.innerHTML='';
}

// ── main builder ──────────────────────────────────────────────
function buildLiveDemo(role) {
  const isReadOnly = role==='analyst';
  const isPassenger= role==='passenger';
  const isDriver   = role==='driver';
  const isDBA      = role==='dba';

  const sel = (id,opts,dflt='') => `<select id="${id}">${dflt?`<option value="">${dflt}</option>`:''}`
    + opts.map(([v,t])=>`<option value="${v}">${t}</option>`).join('') + '</select>';

  // operation panels — filtered by role
  const ops = [];

  if (!isReadOnly) {
    // sp_register_user — passenger + dba
    if (isPassenger||isDBA) ops.push(`
      <div class="demo-op">
        <div class="demo-op-hdr">👤 <strong>sp_register_user</strong> <span>Insert new passenger</span></div>
        <div class="demo-op-body">
          <div class="df-row"><label>First</label><input id="dm-reg-first" value="Sarah"/></div>
          <div class="df-row"><label>Last</label><input id="dm-reg-last" value="Connor"/></div>
          <div class="df-row"><label>Email</label><input id="dm-reg-email" value="sarah.connor@email.com"/></div>
          <div class="df-row"><label>Phone</label><input id="dm-reg-phone" value="555-9999"/></div>
          <button class="demo-btn db-green" onclick="demoOps.register()">EXEC sp_register_user</button>
        </div>
      </div>`);

    // INSERT INTO rides — passenger + dba
    if (isPassenger||isDBA) ops.push(`
      <div class="demo-op">
        <div class="demo-op-hdr">➕ <strong>INSERT INTO rides</strong> <span>Book a ride · BR-1, BR-6</span></div>
        <div class="demo-op-body">
          <div class="df-row"><label>Passenger</label><select id="dm-book-uid"></select></div>
          <div class="df-row"><label>Driver (Available)</label><select id="dm-book-did"></select></div>
          <div class="df-row"><label>From</label><select id="dm-book-slid"></select></div>
          <div class="df-row"><label>To</label><select id="dm-book-elid"></select></div>
          <div class="df-row"><label>Fare ($)</label><input id="dm-book-fare" type="number" value="22.50" step="0.5"/></div>
          <button class="demo-btn db-gold" onclick="demoOps.bookRide()">INSERT INTO rides</button>
        </div>
      </div>`);

    // sp_complete_ride — driver + dba
    if (isDriver||isDBA) ops.push(`
      <div class="demo-op">
        <div class="demo-op-hdr">✅ <strong>sp_complete_ride</strong> <span>Complete · BR-5, BR-7</span></div>
        <div class="demo-op-body">
          <div class="df-row"><label>Pending Ride</label><select id="dm-comp-rid"></select></div>
          <div class="df-row"><label>Final Fare ($)</label><input id="dm-comp-fare" type="number" value="22.50" step="0.5"/></div>
          <button class="demo-btn db-green" onclick="demoOps.completeRide()">EXEC sp_complete_ride</button>
        </div>
      </div>`);

    // sp_cancel_ride — passenger + driver + dba
    if (!isReadOnly) ops.push(`
      <div class="demo-op">
        <div class="demo-op-hdr">❌ <strong>sp_cancel_ride</strong> <span>Cancel · BR-7</span></div>
        <div class="demo-op-body">
          <div class="df-row"><label>Pending Ride</label><select id="dm-cancel-rid"></select></div>
          <button class="demo-btn db-red" onclick="demoOps.cancelRide()">EXEC sp_cancel_ride</button>
        </div>
      </div>`);

    // sp_apply_promo — passenger + dba
    if (isPassenger||isDBA) ops.push(`
      <div class="demo-op">
        <div class="demo-op-hdr">🏷️ <strong>sp_apply_promo</strong> <span>Apply discount · OUTPUT</span></div>
        <div class="demo-op-body">
          <div class="df-row"><label>Completed Ride</label><select id="dm-promo-rid"></select></div>
          <div class="df-row"><label>Promo Code</label><select id="dm-promo-code"></select></div>
          <div class="df-row"><label>New Fare (OUTPUT)</label><span id="dm-promo-out" style="color:var(--green);font-weight:700">—</span></div>
          <button class="demo-btn db-gold" onclick="demoOps.applyPromo()">EXEC sp_apply_promo</button>
        </div>
      </div>`);

    // INSERT payment — passenger + dba
    if (isPassenger||isDBA) ops.push(`
      <div class="demo-op">
        <div class="demo-op-hdr">💳 <strong>INSERT INTO payments</strong> <span>Record payment · BR-3</span></div>
        <div class="demo-op-body">
          <div class="df-row"><label>Ride</label><select id="dm-pay-rid"></select></div>
          <div class="df-row"><label>Amount ($)</label><input id="dm-pay-amount" type="number" value="22.50" step="0.5"/></div>
          <div class="df-row"><label>Method</label><select id="dm-pay-method"><option>Cash</option><option>Card</option><option>Online</option></select></div>
          <button class="demo-btn db-cyan" onclick="demoOps.insertPayment()">INSERT INTO payments</button>
        </div>
      </div>`);

    // INSERT rating — passenger + driver + dba
    ops.push(`
      <div class="demo-op">
        <div class="demo-op-hdr">⭐ <strong>INSERT INTO ratings</strong> <span>Rate ride · BR-4, UNIQUE</span></div>
        <div class="demo-op-body">
          <div class="df-row"><label>Unrated Ride</label><select id="dm-rate-rid"></select></div>
          <div class="df-row"><label>Driver Rating (1-5)</label><select id="dm-rate-dr"><option>5</option><option>4</option><option>3</option><option>2</option><option>1</option></select></div>
          <div class="df-row"><label>User Rating (optional)</label><select id="dm-rate-ur"><option value="">NULL</option><option>5</option><option>4</option><option>3</option><option>2</option><option>1</option></select></div>
          <button class="demo-btn db-gold" onclick="demoOps.insertRating()">INSERT INTO ratings</button>
        </div>
      </div>`);
    ...
    if (isDBA) ops.push(`
      <div class="demo-op">
        <div class="demo-op-hdr">🛡️ <strong>DELETE FROM rides</strong> <span>Test BR-2 — should ROLLBACK if Completed</span></div>
        <div class="demo-op-body">
          <div class="df-row"><label>Ride</label><select id="dm-del-rid"></select></div>
          <button class="demo-btn db-red" onclick="demoOps.deleteRide()">DELETE FROM rides</button>
          <p style="font-size:10px;color:var(--t3);margin-top:6px">Completed → trg_prevent_delete_completed fires ROLLBACK</p>
        </div>
      </div>`);
  }

  // sp_available_drivers — all roles
  ops.push(`
    <div class="demo-op">
      <div class="demo-op-hdr">🚗 <strong>sp_available_drivers</strong> <span>Find available drivers</span></div>
      <div class="demo-op-body">
        <div class="df-row"><label>City (optional)</label>
          <select id="dm-avail-city"><option value="">All cities</option>
            <option>New York</option><option>Chicago</option><option>Los Angeles</option><option>San Francisco</option>
          </select>
        </div>
        <button class="demo-btn db-cyan" onclick="demoOps.availDrivers()">EXEC sp_available_drivers</button>
      </div>
    </div>`);

  // sp_get_user_rides — all roles
  ops.push(`
    <div class="demo-op">
      <div class="demo-op-hdr">📋 <strong>sp_get_user_rides</strong> <span>All rides for a user</span></div>
      <div class="demo-op-body">
        <div class="df-row"><label>User</label><select id="dm-ur-uid"></select></div>
        <button class="demo-btn db-acc" onclick="demoOps.userRides()">EXEC sp_get_user_rides</button>
      </div>
    </div>`);

  // sp_monthly_revenue — analyst + dba
  if (isReadOnly||isDBA) ops.push(`
    <div class="demo-op">
      <div class="demo-op-hdr">📊 <strong>sp_monthly_revenue</strong> <span>Revenue report</span></div>
      <div class="demo-op-body">
        <div class="df-row"><label>Year</label><input id="dm-rev-year" value="2024" type="number"/></div>
        <div class="df-row"><label>Month</label>
          <select id="dm-rev-month"><option value="1">January</option><option value="2">February</option><option value="3">March</option><option value="4">April</option></select>
        </div>
        <button class="demo-btn db-acc" onclick="demoOps.monthlyRevenue()">EXEC sp_monthly_revenue</button>
      </div>
    </div>`);

  const roleBanner = isReadOnly
    ? `<div class="demo-role-banner" style="border-color:var(--yellow);background:rgba(245,158,11,.08)">
         <strong style="color:var(--yellow)">ride_report</strong> — SELECT only · Read-only mode · No INSERT, UPDATE or DELETE access
       </div>`
    : `<div class="demo-role-banner" style="border-color:var(--acc);background:var(--accL)">
         <strong style="color:var(--acc)">${role==='dba'?'ride_dba — db_owner · Full control':role==='driver'?'ride_app · Driver Portal':'ride_app · App User'}</strong>
         — SELECT + INSERT + UPDATE · DELETE explicitly DENIED${isDBA?' (override: db_owner)':''}
       </div>`;

  return `
    <div class="demo-root">
      ${vhdr('Live SQL Demo','Run stored procedures and statements — triggers fire automatically in real time')}
      ${roleBanner}
      <div class="demo-layout">
        <!-- ops -->
        <div class="demo-ops">${ops.join('')}</div>
        <!-- tables -->
        <div class="demo-tables">
          <div class="demo-stats-bar" id="demo-stats"></div>
          <div class="demo-tabs">
            ${['rides','drivers','users','payments','ratings','promos'].map(t=>
              `<button class="demo-tab${t==='rides'?' active':''}" id="dm-tab-${t}" data-t="${t}" onclick="demoSwitchTab('${t}')">${t}</button>`
            ).join('')}
          </div>
          <div id="demo-tbl-body"></div>
        </div>
        <!-- console -->
        <div class="demo-console-wrap">
          <div class="demo-console-hdr">
            <span>📟 SQL Console</span>
            <button onclick="demoClearConsole()" style="font-size:10px;color:var(--t3);background:none;border:1px solid var(--b1);border-radius:5px;padding:2px 8px;cursor:pointer">Clear</button>
          </div>
